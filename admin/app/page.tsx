...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto font-roboto">
      {/* Metric Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Total Recipes */}
        <div className="clean-card p-5 rounded-3xl border border-slate-200/80 clean-card-hover font-khmer">
          ...
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-slate-900">{stats?.totalRecipes ?? 0}</span>
        ...
        <div className="clean-card p-5 rounded-3xl border border-slate-200/80 clean-card-hover font-khmer">
          ...
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-slate-900">{stats?.totalCategories ?? 0}</span>
        ...
        <div className="clean-card p-5 rounded-3xl border border-slate-200/80 clean-card-hover font-khmer">
          <div className="flex items-center justify-between">
            ...
            <div className="w-10 h-10 rounded-2xl bg-yellow-100 text-yellow-700 flex items-center justify-center font-bold">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 tracking-tight font-khmer flex items-center gap-2">
          ...
          <Link href="/recipes" className="text-xs font-bold text-amber-600 hover:text-amber-700 flex items-center gap-1 font-khmer">
            ...
            {featuredList.map((dish) => (
              <div key={dish.id} className="clean-card rounded-3xl overflow-hidden clean-card-hover flex flex-col justify-between border border-slate-200">
                <div className="relative h-48 w-full bg-slate-100 overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-white/95 backdrop-blur-md text-amber-800 text-[11px] font-extrabold capitalize shadow-sm border border-amber-100 font-khmer">
                    {dish.categorySlug}
                  </div>
                  <span className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-amber-500 text-white text-[10px] font-extrabold uppercase shadow-sm font-roboto">
                    {t('featuredBadge')}
                  </span>
                </div>
                ...
                <div className="p-5 space-y-3 flex-1 flex flex-col justify-between">
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 font-khmer">
                    <span className="flex items-center gap-1.5 font-medium bg-amber-50 px-2.5 py-1 rounded-xl text-amber-900 border border-amber-200/60 font-roboto">
      ...
      <div className="clean-card rounded-3xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
          <div>
            <h3 className="font-extrabold text-slate-900 text-base flex items-center gap-2 font-khmer">
          ...
          <Link
            href="/recipes/new"
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-2xl shadow-sm transition-all self-start sm:self-auto font-khmer"
          >
